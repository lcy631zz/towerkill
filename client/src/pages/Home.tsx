...
const PRESETS: Record<"custom" | "local", { label: string; baseUrl: string; model: string; price?: { inputPerM: number; outputPerM: number; currency: string; note: string } }[]> = {
  custom: [
    { label: "DeepSeek V4 Flash（快而省）", baseUrl: "https://api.deepseek.com", model: "deepseek-v4-flash", price: { inputPerM: 1.5, outputPerM: 4.5, currency: "元", note: "每百万 tokens，高峰时段翻倍" } },
    { label: "DeepSeek V4 Pro（更强）", baseUrl: "https://api.deepseek.com", model: "deepseek-v4-pro", price: { inputPerM: 4.5, outputPerM: 13.5, currency: "元", note: "每百万 tokens，高峰时段翻倍" } },
    { label: "硅基流动 DeepSeek-V3", baseUrl: "https://api.siliconflow.cn/v1", model: "deepseek-ai/DeepSeek-V3" },
    { label: "Kimi moonshot-v1-32k", baseUrl: "https://api.moonshot.cn/v1", model: "moonshot-v1-32k" },
    { label: "OpenAI gpt-4.1-mini", baseUrl: "https://api.openai.com/v1", model: "gpt-4.1-mini", price: { inputPerM: 0.4, outputPerM: 1.6, currency: "$", note: "美元参考价，每百万 tokens" } },
  ],
  local: [
    { label: "Qwen3 8B（默认推荐）", baseUrl: "http://127.0.0.1:11434/v1", model: "qwen3:8b" },
    { label: "Qwen3 4B（低配机）", baseUrl: "http://127.0.0.1:11434/v1", model: "qwen3:4b" },
    { label: "DeepSeek-R1 8B 蒸馏", baseUrl: "http://127.0.0.1:11434/v1", model: "deepseek-r1:8b" },
    { label: "DeepSeek-R1 14B 蒸馏", baseUrl: "http://127.0.0.1:11434/v1", model: "deepseek-r1:14b" },
  ],
};
...
export default function Home() {
  ...
  async function streamInterpretation(nextResult: RitualResult) {
    setStreaming(true); setInterpretation(""); setNotice(""); setStreamStatus("连接解读服务中…"); setUsage(null);
  ...
  const matchedPreset = mode === "custom" ? PRESETS.custom.find((preset) => preset.baseUrl === provider.baseUrl && preset.model === provider.model) : undefined;
  const priceHint = mode === "rules" ? "" : mode === "local" ? "本地模型不产生费用，仅占用本机算力。" : matchedPreset?.price ? `参考资费（空闲时段）：输入 ${matchedPreset.price.inputPerM}${matchedPreset.price.currency} · 输出 ${matchedPreset.price.outputPerM}${matchedPreset.price.currency} / 每百万 tokens · ${matchedPreset.price.note}` : "参考资费以服务商官网为准，本应用不估算费用。";
  const usageCost = (() => {
    if (!usage) return "";
    if (mode === "local") return "本地模型不产生费用";
    if (!matchedPreset?.price) return "费用以服务商账单为准";
    const cost = (usage.promptTokens * matchedPreset.price.inputPerM + usage.completionTokens * matchedPreset.price.outputPerM) / 1_000_000;
    return `估算费用 ${matchedPreset.price.currency}${cost < 0.01 ? "<0.01" : cost.toFixed(2)} · ${matchedPreset.price.note}`;
  })();
  ...
  return <div className="win-app">
    ...
    <aside className="win-right"><div className="version"><b>天意演策 0.2</b><span>本机模式</span></div><div className="version"><b>牌池</b><span>156 张</span></div><div className="version"><b>解读器</b><span>规则本机</span></div></aside>
  </div>;
}
