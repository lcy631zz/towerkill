...
export function OracleCard({ card, orientation, index, assetUrl }: Props) {
  ...
  if (assetUrl) return <article data-orientation={orientation} className={`sgs-card oracle-card sgs-card--${card.kind} sgs-card--image ${reversed ? "oracle-card--reversed" : ""}`} aria-label={`第 ${index} 张：${card.name}，${reversed ? "逆位倒着" : "正位正着"}`}>
    <img className="sgs-card__image" src={assetUrl} alt={`${card.name} 本地卡图`} />
  </article>;

  return <article data-orientation={orientation} className={`sgs-card oracle-card sgs-card--${card.kind} ${reversed ? "oracle-card--reversed" : ""}`} aria-label={`第 ${index} 张：${card.name}，${reversed ? "逆位倒着" : "正位正着"}`}>
