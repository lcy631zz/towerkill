...
三张结果牌采用原创的文字化卡面：保留原型桌游式字段层级，包括牌类、牌名、类别、花色/点数、技能、效果、典故、象征关键词与明确的正位/逆位状态。页面不使用官方插画、牌背或其他官方视觉资产。
...
当前测试为 **5 个测试文件、14 条断言**，覆盖八卦规则、牌池实体数量、抽牌接口、文字/图片卡面正逆位、正逆方向分布、标准卡式顶栏/插画窗/说明区结构、素材实体 ID 校验、路径越界拦截与缺失图片回退。另已通过本机规则模式的 SSE 接口验证，确认该模式返回“未调用任何大模型”的通知与完成事件。

抽牌方向恢复为**每张牌独立随机判定**，不会为了保证画面中出现逆位而修改结果。端到端 DOM 核验已确认逆位会带有 `oracle-card--reversed` 类并整张旋转；卡面不显示额外的正位、逆位或倒着标签。
...
牌库口径、来源和原创卡面边界见 [`DATA_NOTES.md`](./DATA_NOTES.md)。正逆位设定与“六壬意象旁注”均为本项目的娱乐解读层；后者不等同于包含干支、月将、四课、三传的传统完整大六壬排盘。
