# 参考无名杀工程结构：天意演策桌面版实施说明

> **原则**：本方案只借鉴工程分层与桌面端组织思想，不复制无名杀源代码、第三方卡图、具体卡牌文案、音频或扩展包。本文件是工程实现说明，不构成法律意见。

## 1. 无名杀当前的结构给出的启发

无名杀主仓库采用多应用组织：`apps/core` 为核心内容，另有 `apps/electron` 和 `apps/mobile` 作为不同平台外壳；核心目录中将 `card`、`character`、`extension`、`game`、`image`、`layout`、`mode`、`theme` 与类型定义分开。[1] [2] 对天意演策而言，可借鉴的不是游戏数据或具体实现，而是这条边界：**领域数据、界面、平台壳、可选资产和扩展配置彼此分离**。

| 无名杀的通用分层 | 天意演策的独立对应层 | 是否可直接复制 |
|---|---|---:|
...
这是当前项目最合适的路径。天意演策保留自己的 React、Express 与 Electron 代码，仅按如下方式继续整理：
...
若要直接复制、修改或将无名杀代码合并到天意演策，必须先把它当作 GPL-3.0 代码处理。无名杀 README 明示项目基于 GPL-3.0，并要求二次打包和分发保留出处、不要用于商业用途。[1]
...
即使满足 GPL-3.0，仍不能因此取得无名杀仓库中不属于其开源授权范围的第三方素材权利；官方三国卡牌卡图与特定文案应另行处理。
...
对“天意演策”而言，第 1–4 步足以完成所需的占卜功能和本地素材显示；没有功能性理由必须复制无名杀代码。因此建议先走**路径 A**。
